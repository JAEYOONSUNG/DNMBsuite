...
if [ "${DNMB_ENTRYPOINT_SKIP_ROOT_SETUP:-0}" != "1" ]; then
  ...
  if [ -d "$EGGNOG_CACHE" ]; then
    EMAPPER_DATA=$(/opt/biotools/envs/eggnog/bin/python -c "import os,eggnogmapper;print(os.path.join(os.path.dirname(eggnogmapper.__file__),'data'))" 2>/dev/null || true)
